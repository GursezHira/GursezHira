...
import { unitConnectorBottom, nodeConnectorTop } from './tree-layout.js';

/**
 * Build a stroke-colour lookup from TREE_DATA branch themes
 * plus the fixed Gursez gold.
 */
function buildStrokeMap(TREE_DATA) {
  const map = { Gursez: 'rgba(201,169,110,0.75)' };
 ...
 */
function getUnitStroke(u, nodeMap, sideStrokeMap) {
 ...
 */
function drawBezier(svg, x1, y1, x2, y2, stroke) {
  const cy = (y1 + y2) / 2;
 ...
 *
 * @param {{ units, unitByNode, unitPos, canvasW, canvasH, nodeMap }} layout
 * @param {object} TREE_DATA
 */
export function renderConnectors(layout, TREE_DATA) {
